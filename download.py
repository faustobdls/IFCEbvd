...
def dump(matricula, senha, id_book):
    ...
    while p_1 == 0:
        try:
            ...
            if p_1 == 1:
                ...
                page_download(phantom.execute_script('return (RDP.reader.currentPages[0].portrait.zoom[%s])' % zoom), 'book/%s-00000.jpg' % id_book)
        except:
            p_1 = 0
    ...
    p0 = phantom.execute_script('return (RDP.reader.currentPages[0].portrait.zoom[%s])' % zoom)
    page_download(p0, 'book/%s-00000.jpg' % id_book)
    ...
    for i in range(1, num_pag, 2):
        ...
        while (p_1 == 0) or (p_2 == 0):
            try:
                ...
                p_1 = phantom.execute_script("if (RDP.reader.currentPages[0].portrait.zoom[%s]) { return 1 } else { return 0 }" % zoom)
                p_2 = phantom.execute_script("if (RDP.reader.currentPages[0].portrait.zoom[%s]) { return 1 } else { return 0 }" % zoom)
            except:
                p_1 , p_2 = 0, 0
        # carregou, pegar endereco
        _p1 = phantom.execute_script('return (RDP.reader.currentPages[0].portrait.zoom[%s])' % zoom)
        _p2 = phantom.execute_script('return (RDP.reader.currentPages[1].portrait.zoom[%s])' % zoom)
        ...
        page_download(_p1, 'book/%s-%05d.jpg' % (id_book, i))
        page_download(_p2, 'book/%s-%05d.jpg' % (id_book, i+1))
...
if __name__ == "__main__":
    if len(sys.argv) == 1:
        print("Falta fornecer parâmetros!\n"
              "Sintaxe: download.py <funcao> <número da matrícula> <senha> <endereco url do livro no bvu> [endereco url de outro livro] ..")
        exit()
